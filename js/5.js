(function ($) {
  ...
  jQuery(document).ready(function ($) {
    ...
    function change_name_and_ids(item, field_end = null, id_rand = "") {
      ...
      field_end
        .find(".field-repeater-data")
        .attr("data-old", JSON.stringify(datas));
      item = $(item);
      ...
      if (!push_name) {
        ...
        if (list_names.length > 0) {
          $('input[name="_yeeaddons_cf7_repeater_fields"]').val(
            list_names.join("|")
          );
        }
      }
    ...
    function add_repeater_data(button) {
      ...
      var item = $(
        '<div class="repeater-field-item"><div class="repeater-field-header"></div><div class="repeater-field-content"></div></div>'
      );
    ...
    function get_repeater_data(step_field, id_rand) {
      var html_step = change_name_and_ids(
        step_field.find(".cf7-field-repeater-data-html").html(),
        step_field,
        id_rand
      );
      return html_step;
    ...
    function get_repeater_data_name() {
      ...
      $(".cf7-repeater").each(function () {
        var datas =
          $(this).find(".field-repeater-data").data("datas") +
          " yeeaddons_test";
        ...
        step_field_data.val(
          JSON.stringify({ count: 1, fields: names, id: [] })
        );
        step_field_data.attr(
          "data-old",
          JSON.stringify({ count: 1, fields: names, id: [] })
        );
        var initial_rows = 0;
        ...
        if (cf7_repeater.pro == "ok") {
          ...
          var initial_rows_map_number = $(
            '[name="' + initial_rows_map_field + '"]'
          ).val();
          if (initial_rows_map_number > 0) {
            $('[name="' + initial_rows_map_field + '"]').attr(
              "data-repeater",
              $(this).find(".field-repeater-data").attr("name")
            );
            $('[name="' + initial_rows_map_field + '"]').attr(
              "repeater_initial_rows",
              "ok"
            );
    ...
    $("body").on("change", "[repeater_initial_rows='ok']", function (e) {
      if (cf7_repeater.pro == "ok") {
        ...
        $("#" + repeater_id)
          .find(".repeater-field-item")
          .remove();
        var number = $(this).val();
        for (let i = 0; i < number; i++) {
          $("#" + repeater_id)
            .find(".repeater-field-button-add")
            .click();
        }
      }
    });
    ...
    function check_max_row(step_field) {
      var max = explode_datas(
        step_field.find(".field-repeater-data").data("datas")
      );
    ...
    $("body").on(
      ...
      function (e) {
        ...
        $(this)
          .closest(".repeater-field-item")
          .find(".repeater-field-content")
          .slideToggle("slow");
        $(this)
          .closest(".repeater-field-item")
          .find(".repeater-field-header")
          .toggleClass("repeater-content-show");
      }
    );
    $("body").on(
      "click",
      ...
      function (e) {
        ...
        if (check_min_row($(this).closest(".cf7-repeater"))) {
          ...
          var datas = JSON.parse(
            $(this).closest(".cf7-repeater").find(".field-repeater-data").val()
          );
          var datas_ids = datas.id;
          datas_ids = removeAR(datas_ids, id);
          datas.id = datas_ids;
          $(this)
            .closest(".cf7-repeater")
            .find(".field-repeater-data")
